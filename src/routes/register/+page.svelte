<script>
  ...
  let email = $state('');
  let passwort = $state('');
  let passwortBestaetigen = $state('');
  let fehler = $state('');
  let laedt = $state(false);

  /**
   * Schickt das Registrierungs-Formular ab.
   * @param {SubmitEvent} e
   */
  async function abschicken(e) {
    ...
    // --- Client-seitige Validierung VOR dem Fetch ---
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(daten.fehler ?? 'Registrierung fehlgeschlagen');
      }
...
<svelte:head>
  <title>Registrieren – Taskly</title>
</svelte:head>
...
<div class="seite">
  <div class="karte">
    ...
    <p class="untertitel">Konto erstellen</p>
    ...
    <form onsubmit={abschicken}>
      ...
      <label class="feld">
        <span>Passwort <small>(mind. 8 Zeichen)</small></span>
        <input
          type="password"
          bind:value={passwort}
          required
      ...
      <label class="feld">
        ...
        <input
          type="password"
          bind:value={passwortBestaetigen}
          required
          minlength="8"
          autocomplete="new-password"
        />
      ...
      <button type="submit" class="btn-primaer voll" disabled={laedt}>
        {laedt ? 'Wird erstellt…' : 'Konto erstellen'}
      </button>
    </form>
